(function () {
  "use strict";
  let SimpleUndo = function (options) {
...
DrawingBoard.Utils.pixelAt = function (image, x, y) {
  ...
  let c = DrawingBoard.Utils.RGBToInt(
    image.data[i],
    image.data[i + 1],
    image.data[i + 2]
  );
...
DrawingBoard.Board.prototype = {
  ...
  resetBackground: function (background, callback) {
    ...
    let bgIsColor = DrawingBoard.Utils.isColor(background);
    let prevMode = this.getMode();
    this.setMode("pencil");
  ...
  resize: function () {
    ...
    let that = this;
    let sum = function (values, multiplier) {
  ...
  initControls: function () {
    ...
    for (let i = 0; i < this.opts.controls.length; i++) {
      ...
      if (typeof this.opts.controls[i] == "string")
        ...
        for (let controlName in this.opts.controls[i]) break;
  ...
  addControl: function (control, optsOrPos, pos) {
    ...
    let opts = typeof optsOrPos == "object" ? optsOrPos : {};
  ...
  setImg: function (src, opts) {
    ...
    let ctx = this.ctx;
    let img = new Image();
    let oldGCO = ctx.globalCompositeOperation;
    img.onload = function () {
  ...
  setColor: function (color) {
    ...
    if (this.opts.eraserColor !== "transparent" && this.mode === "eraser") {
      let setStrokeStyle = function (mode) {
  ...
  fill: function (e) {
    ...
    let r = parseInt(stroke.substr(1, 2), 16);
    let g = parseInt(stroke.substr(3, 2), 16);
    let b = parseInt(stroke.substr(5, 2), 16);

    // starting point
    let start = DrawingBoard.Utils.pixelAt(
      img,
      parseInt(e.coords.x, 10),
      parseInt(e.coords.y, 10)
    );
    let startColor = start[COLOR];
    let tolerance = this.opts.fillTolerance;
    let useHack = this.opts.fillHack;
  ...
  draw: function () {
    ...
    if (
      ...
      let transform = DrawingBoard.Utils.tpl(
    ...
    if (this.isDrawing) {
      let currentMid = this._getMidInputCoords(this.coords.current);
      this.ctx.beginPath();
...
DrawingBoard.Control.Color = DrawingBoard.Control.extend({
  ...
  initialize: function () {
    ...
    $("body").on("click", function (e) {
      let $target = $(e.target);
      let $relatedButton = $target.hasClass(
        "drawing-board-control-colors-current"
      )
        ? $target
        : $target.closest(".drawing-board-control-colors-current");
      let $myButton = that.$el.find(".drawing-board-control-colors-current");
      let $popup = that.$el.find(".drawing-board-control-colors-rainbows");
      if (
