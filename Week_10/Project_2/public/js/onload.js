let username;
let enemyid;
let greens = {main: '#11CF00', extra: '#D711ED', explosion: ['lime','#2BB31E','#1C8212']};
let purples = {main: '#D711ED', extra: '#11CF00', explosion: ['#D711ED','#AC0DBD','#6B0876']};
...
$(function() {
	...
	$('#splashbutton').on('click', function() {
		...
		socket.emit('welcome message', "Welcome to 90's Tank Game. " +
	...
	$('#login-input').keypress(function(event) {
		let allUsersNames = $('.userscontainer').find('p');
		let badName = false;
		if (event.keyCode === 13) {
	...
	$('body').on('click', '.challenge-button', function() {
		let messageContainer = $('.chatmessagescontainer');
		let sentChallenge = $('<p>').addClass('message').addClass('challenge-message');
		let challengee = $(this).parent().find('p').text();
	...
	$('body').on('click', '.challenge-message-button', function(event) {
		let enId = $(this).attr('invitation-id');
		$(this).remove();
	...
	$('canvas').mousemove(function(event) {
		let extraTop = $('canvas').offset().top + parseInt($('canvas').css('border-top-width'));
		let extraLeft = $('canvas').offset().left + parseInt($('canvas').css('border-left-width'));
		if (!myTank.gameOver) {
